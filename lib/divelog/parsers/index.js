...
import { garminFitParser } from './garmin-fit.js';
import { csvParser } from './csv.js';

/** @type {import('../types.js').ParserModule[]} — csv last: weakest sniff. */
export const PARSERS = [uddfParser, subsurfaceParser, suuntoSmlParser, garminFitParser, csvParser];
...
export function detectFormat(bytes, fileName) {
  ...
  if (looksBinary(bytes)) {
    // binary formats sniff on bytes alone (FIT magic); text stays empty
    for (const p of PARSERS) {
      try {
        if (p.sniff(bytes, '')) return { parser: p, text: '', via: 'sniff' };
      } catch { /* sniffers must not break detection */ }
    }
    return { error: unsupportedBinary(bytes) };
  }
