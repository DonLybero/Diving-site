...
function convert(value, kind, unit, warnings, label) {
  ...
  if (kind === 'temp') {
    ...
    if (value > 45) { warnings.push(`${label}: temperature ${value} looks like °F — converted`); return fToC(value); }
    return value;
  }
  if (kind === 'pressure') {
...
function euroDecimals(delimiter, cell) {
  // ';'-delimited files pair with decimal commas: '18,4' → '18.4'
  if (delimiter !== ';') return cell;
  return typeof cell === 'string' ? cell.replace(/(\d),(\d)/g, '$1.$2') : cell;
}
...
function buildDive(cells, cols, ctx, warnings, rowNo) {
  ...
  const get = (field) => {
    ...
    const v = cells[idx];
    return v === undefined || String(v).trim() === '' ? undefined : euroDecimals(ctx.delimiter, String(v).trim());
  };
  ...
  const at = convert(num(get('airTemp')), 'temp', ctx.units[cols.airTemp], warnings, label);
...
function parseProfileCsv(rows, cols, ctx, result) {
  ...
  rows.forEach((cells, i) => {
    ...
    const depthM = convert(num(euroDecimals(ctx.delimiter, cells[cols.sampleDepth])), 'depth', ctx.units[cols.sampleDepth], result.warnings, `row ${i + 2}`);
    if (tSec === undefined || depthM === undefined) return;
    const s = { tSec, depthM };
    if (cols.sampleTemp !== undefined) {
      const t = convert(num(euroDecimals(ctx.delimiter, cells[cols.sampleTemp])), 'temp', ctx.units[cols.sampleTemp], result.warnings, `row ${i + 2}`);
      if (t !== undefined) s.tempC = t;
    }
    if (cols.samplePressure !== undefined) {
      const p = convert(num(euroDecimals(ctx.delimiter, cells[cols.samplePressure])), 'pressure', ctx.units[cols.samplePressure], result.warnings, `row ${i + 2}`);
