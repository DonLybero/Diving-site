...
//   deleteDive(id)
//   clearAll()                "delete all my dives": wipes dives, imports and
//                             remembered CSV mappings (derived from user files);
//                             the unit preference is not personal data and stays
...
export class MemoryStore {
  ...
  async clearAll() {
    this.dives.clear();
    this.imports.clear();
    for (const k of [...this.settings.keys()]) if (k.startsWith('csvMapping:')) this.settings.delete(k);
  }
...
export class IndexedDBStore {
  ...
  async clearAll() {
    const tx = this.db.transaction(['dives', 'imports', 'settings'], 'readwrite');
    tx.objectStore('dives').clear();
    tx.objectStore('imports').clear();
    const settings = tx.objectStore('settings');
    const keysReq = settings.getAllKeys();
    keysReq.onsuccess = () => {
      for (const k of keysReq.result) if (String(k).startsWith('csvMapping:')) settings.delete(k);
    };
    await txDone(tx);
  }
  async putImport(rec) {
