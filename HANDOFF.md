...
| `divelog.html` + `divelog.js` | **Dive log** — client-side logbook (import any dive computer's export, depth-profile charts, UDDF export). Hash-routed views; dives live in the visitor's IndexedDB, nothing uploaded. PRD + architecture decisions: `docs/divelog-prd.md`. |
...
- Dive log: shipped 2026-07 as a client-side v1 (`divelog.html` — UDDF,
  Subsurface XML, Suunto SML, CSV import; profile charts; UDDF export; dives
  stay in the visitor's browser). Still open, in priority order: Garmin FIT
  (vendor-branch Suunto-FIT or reject with a DM5 pointer), Shearwater Cloud
  XML, DAN DL7, "transfer your dive log from [brand]" SEO articles linking to
  the tool, and promoting the footer link into the SPA's tab row (owner call).
